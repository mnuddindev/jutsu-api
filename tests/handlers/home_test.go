...
func TestHomeHandler_GetHomeInfo(t *testing.T) {
	app := fiber.New()
	h := handler.NewHomeHandler()
	app.Get("/api", h.GetHomeInfo)
	app.Get("/api/", h.GetHomeInfo)

	t.Run("Success - Get home info", func(t *testing.T) {
	...
	t.Run("Success - Get home info with trailing slash", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodGet, "/api/", nil)
